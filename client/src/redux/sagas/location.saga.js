...
function* getCityListSaga(action) {
  try {
    const result = yield axios.get("http://localhost:3500/cities");
...
function* getDistrictListSaga(action) {
  try {
    ...
    const result = yield axios.get("http://localhost:3500/districts", {
      params: {
        parentcode: cityCode,
      },
    });
...
function* getWardListSaga(action) {
  try {
    ...
    const result = yield axios.get("http://localhost:3500/wards", {
      params: {
